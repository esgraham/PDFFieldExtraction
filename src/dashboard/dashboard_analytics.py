...
def generate_sample_metrics() -> Tuple[List[DocumentMetrics], List[FieldMetrics]]:
    """Generate sample metrics for demonstration."""
    
    # Sample document metrics
    doc_metrics = []
    for i in range(100):
        doc_id = f"DOC-2024-{i:04d}"
        doc_type = DocumentType.INVOICE if i % 3 == 0 else DocumentType.RECEIPT if i % 3 == 1 else DocumentType.CONTRACT
        
        processing_time = 2.5 + (i % 10) * 0.5  # Vary processing time
        success_rate = 0.92  # 92% success rate
        
        status = ProcessingStatus.SUCCESS if i < success_rate * 100 else ProcessingStatus.HITL_REQUIRED
        hitl_required = status == ProcessingStatus.HITL_REQUIRED
        ...
        doc_metrics.append(DocumentMetrics(
            document_id=doc_id,
            document_type=doc_type,
            processing_status=status,
            start_time=datetime.now() - timedelta(hours=i),
            end_time=datetime.now() - timedelta(hours=i) + timedelta(seconds=processing_time),
            ocr_time=processing_time * 0.4,
            extraction_time=processing_time * 0.3,
            validation_time=processing_time * 0.2,
            total_processing_time=processing_time,
            field_count=8 + (i % 5),
            validation_errors=[],
            confidence_scores={"field1": 0.85 + (i % 10) * 0.01},
            hitl_required=hitl_required,
            hitl_reason="Low confidence fields" if hitl_required else None,
            cost_breakdown={
                "compute": 0.015 + (i % 5) * 0.001,
                "storage": 0.002,
                "review": 0.05 if hitl_required else 0.0
            },
            exception_type=exception_type
        ))
    
    # Sample field metrics
    field_metrics = []
    fields = ["invoice_number", "invoice_date", "total_amount", "vendor_name", "customer_name"]
    doc_types = ["invoice", "receipt", "contract"]
    
    for field in fields:
        for doc_type in doc_types:
            # Simulate realistic field performance
            precision = 0.88 + (hash(field + doc_type) % 20) * 0.005
            recall = 0.85 + (hash(field + doc_type) % 25) * 0.004
            ...
            field_metrics.append(FieldMetrics(
                ...
                true_positives=int(precision * 100),
                false_positives=int((1 - precision) * 100),
                false_negatives=int((1 - recall) * 100),
                confidence_avg=0.82,
                confidence_std=0.15
            ))
    
    return doc_metrics, field_metrics